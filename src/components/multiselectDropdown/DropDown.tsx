            ...
            <div className="w-full relative" >
                <div
...

                    {
                        value ?
                            (
                                <p className={`text-black sm:text-sm sm:leading-6 font-ubuntu font-medium ${style?.placeholder && style?.placeholder}`}>{returnMatchingLabel({ arrayOfObject: data, value:value[0] })}</p>
                            )
                            :
                            (
                                <>
...
let j = value.indexOf(item.value)
                                                setValue(value.filter((_, i) => i !== j));
                                            }
                                            setIsExpanded(false);
                                        }}
                                    >
                                        <p className={`text-sm ${value?.includes(item?.value) ? 'text-radio' : 'text-neutral-600'} md:text-md font-medium`} >{item?.label}</p>
                                    </div>
                                ))
                            }
                        </div>
